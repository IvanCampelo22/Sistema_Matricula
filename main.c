...
void remover() {
	...
	do {
		...
		if (posicao == - 1) {
			...
			mostre(posicao);
			printf("\n\nDeseja remover o aluno (1-Sim/0-Não)? ");
			scanf("%d", &conf);
			fflush(stdin);
...
// função que altera os dados do aluno
void alterar() {
	int matalt, conf, resp, posicao;

	do{
		cabec();
		printf("\n\nAlterar média do aluno\n\n\n");
		printf("Matricula: ");
		scanf("%d", &matalt);
		fflush(stdin);
		...
		if (posicao == -1) {
			...
			mostre(posicao);
			printf("\n\nAlterar a média do aluno (1-Sim/0-Não)? ");
			scanf("%d", &conf);
			fflush(stdin);
			...
			if (conf == 1) {
				...
				fwrite(&aluno_aux, sizeof(TAluno), 1, paluno);
				mostre(posicao);
				printf("\nMédia do aluno alterada com sucesso!\n");
			} else {
				printf("\n\nAlteração cancelada!\n\n");
			}
		}
		printf("\n\nDeseja altera outro aluno (1-Sim/0-Não)? ");
		scanf("%d", &resp);
		fflush(stdin);
...
int main(int argc, char *argv[]) {
	...
	do {
		...
		printf("4. Alterar média do aluno\n");
